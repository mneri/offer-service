 ...
 */
@Component
class RoleSpecImpl implements RoleSpec {
    /**
     * {@inheritDoc}
     */
    @Override
    public Specification<Role> ownerIdIsEqualTo(UUID userId) {
        return (root, query, builder) -> {
            Subquery<UUID> subQuery = query.subquery(UUID.class);
            Root<User> subRoot = subQuery.from(User.class);
            Join<User, Role> join = subRoot.join(User_.roles);

            subQuery.select(join.get(Role_.id))
                    .where(builder.equal(subRoot.get(User_.id), userId));

            return builder.equal(root.get(Role_.id), subQuery);
        };
    }
}
